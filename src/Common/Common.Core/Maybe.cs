...
namespace Common.Core
{
    public class Maybe<T> where T : class
    {
        ...
        public Maybe<TO> Bind<TO>(Func<T, Maybe<TO>> func) where TO : class => _value != null ? func(_value) : Maybe<TO>.None();

        public T Coalesce(T otherwise) => _value != null? _value : otherwise;
        public T Coalesce(Func<T> otherwise) => _value != null? _value : otherwise();
        public async Task<T> Coalesce(Func<Task<T>> otherwise) => _value != null? _value : await otherwise();
